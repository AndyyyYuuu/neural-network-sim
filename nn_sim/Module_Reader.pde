...
public class GraphModule extends Module{
  ...
  public void draw(){
    ...
    textSize(11);
    if (hasAllInputs() && getInput(0).forward() != null){
      text(processDouble(getInput(0).forward().getValue()), this.pos.x-70, this.pos.y-56);
    }
    drawAttachments(COLOR_READER);
    if (min != null && max != null && values.size() > 1){
  ...
  public float yPos(Num y){
    return (float)(this.pos.y+65 - 105*((y.getValue()-min)/(max-min)));
  }
  
  public float xPos(int x, int xMax){
    return (float)(this.pos.x-70 + 140*((double)x/(double)(xMax-1)));
  }
